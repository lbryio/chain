...
// newAddressPubKey returns a new lbcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) lbcutil.Address {
	addr, err := lbcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new lbcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) lbcutil.Address {
	addr, err := lbcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new lbcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) lbcutil.Address {
	addr, err := lbcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the lbcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the lbcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := lbcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := lbcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := lbcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := lbcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := lbcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*lbcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*lbcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*lbcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*lbcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*lbcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
