...
import (
	...
	"github.com/lbryio/lbrycrd.go/rpcclient"
...
type memWallet struct {
	...
	addrs map[uint32]lbcutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]lbcutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*lbcutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          lbcutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (lbcutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []lbcutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt lbcutil.Amount,
	feeRate lbcutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := lbcutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate lbcutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate lbcutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt lbcutil.Amount
	for _, output := range outputs {
		outputAmt += lbcutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() lbcutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance lbcutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (lbcutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := lbcutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
