...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt lbcutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(lbcutil.Amount(5 * lbcutil.SatoshiPerBitcoin))
	...
	txid = genSpend(lbcutil.Amount(500 * lbcutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(lbcutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*lbcutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, lbcutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := lbcutil.Amount(250 * lbcutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = lbcutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := lbcutil.Amount(50 * lbcutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := lbcutil.Amount(numMatureOutputs * 50 * lbcutil.SatoshiPerBitcoin)
