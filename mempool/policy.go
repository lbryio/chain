...
const (
	...
	DefaultMinRelayTxFee = lbcutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee lbcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > lbcutil.MaxSatoshi {
		minFee = lbcutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *lbcutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee lbcutil.Amount) bool {
...
func checkTransactionStandard(tx *lbcutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee lbcutil.Amount,
...
func GetTxVirtualSize(tx *lbcutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
