...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: lbcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := lbcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*lbcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, lbcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount lbcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee lbcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee lbcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee lbcutil.Amount) *wire.MsgTx {
