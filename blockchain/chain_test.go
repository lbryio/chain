...
func _TestHaveBlock(t *testing.T) {
	...
	_, isOrphan, err := chain.ProcessBlock(lbcutil.NewBlock(&Block100000),
		BFNone)
	if err != nil {
...
func TestCalcSequenceLock(t *testing.T) {
	...
	// age of 4 blocks.
	targetTx := lbcutil.NewTx(&wire.MsgTx{
		TxOut: []*wire.TxOut{{
			PkScript: nil,
			Value:    10,
		}},
	...
	utxoView.AddTxOuts(lbcutil.NewTx(unConfTx), 0x7fffffff)
